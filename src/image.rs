...
impl ImageConverter {
    ...
    pub fn convert(
        ...
        let (converted, mime_type) = match target_format {
            OutputFormat::Avif if self.enable_avif => (self.to_avif(&img)?, "image/avif"),
            OutputFormat::WebP if self.enable_webp => (self.to_webp(&img)?, "image/webp"),
            OutputFormat::Jpeg => (self.to_jpeg(&img)?, "image/jpeg"),
            OutputFormat::Png => (self.to_png(&img)?, "image/png"),
...
mod tests {
    ...
    fn test_format_from_content_type() {
        ...
        assert_eq!(
            format_from_content_type("image/jpg"),
            Some(OutputFormat::Jpeg)
        );
        assert_eq!(
